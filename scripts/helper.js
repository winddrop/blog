...
export function organizeData(data) {
  ...
  data.forEach((item) => {
    ...
    for (let i = 1; i < category.length; i++) {
      ...
      let existingCat = currentItems.find(
        (item) => item.text === cat && !item.link
      );
      if (!existingCat) {
...
export function organizeNav(data) {
  ...
  data.forEach((item) => {
    ...
    // 存储二级分类（去重）
    categoryMap[level1].add({
      level2: level2,
      halfPath: `/${categoryPath}/`,
      fullPath: `/${categoryPath}/${item.title}/`,
    });
  });
  ...
  return Object.entries(categoryMap).map(([level1Text, level2Set]) => {
    return {
      ...
      items: Array.from(level2Set).map((level2Obj) => {
        // 生成link和activeMatch：/一级/二级/
        // const path = `/${categoryPath}/${category.title}/`;
        return {
          text: level2Obj.level2,
          link: `${level2Obj.fullPath}`,
          activeMatch: `${level2Obj.halfPath}`,
        };
      }),
    };
  });
